...
@section('content')
    <div class="py-3">
        ...
        @php
            ...
            // 1) metas por indicador (y opcionalmente por formulario)
            ...
            // 2) última carga por (id_form + id_meta)
            $ultimasCargas = Carga::selectRaw('id_form, id_meta, MAX(id_carga) as ultima_id')
                ->whereIn('id_form', $formularios->pluck('id_form')->unique())
                ->whereNotNull('id_meta')
                ->groupBy('id_form', 'id_meta')
                ->get();

            $cargasUltimasPorMeta = Carga::whereIn('id_carga', $ultimasCargas->pluck('ultima_id'))
                ->get()
                ...
                <div class="col-12 col-md-6 col-xl-4 mb-3">
                    <div class="card shadow-sm border-0 h-100" style="border-radius:14px;">
                        <div class="card-body">
                            ...
                            @if ($form->indicador)
                                @php
                                    $metas = $metasPorIndicador[$form->id_ind] ?? collect();
                                        ...
                                        <div class="border rounded p-2" style="background:#fafafa;">
                                            ...
                                            @foreach ($metas as $m)
                                                @php
                                                    ...
                                                    $href =
                                                        route('usuario.formulario.captura', $params) .
                                                        '?id_meta=' .
                                                        $m->id;

                                                    // reglas de botón por meta
                                                    $btnClassMeta = 'btn-success';
                                                    $btnTextMeta = 'Capturar';
                                                    $iconMeta = 'bi-pencil-square';

                                                    if ($st === 'BORRADOR' && $cargaMeta) {
                                                        $href = $href . '&id_carga=' . $cargaMeta->id_carga;
